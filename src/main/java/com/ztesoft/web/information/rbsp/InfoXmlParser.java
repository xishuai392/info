...
public class InfoXmlParser {
    ...
    public static InfoResultVO parserXML(String xml) throws BaseAppException {
        ...
        try {
            ...
            for (Iterator i = root.elementIterator("Method"); i.hasNext();) {
                ...
                for (Iterator<Element> j = method.elementIterator("Items"); j
                        .hasNext();) {
                    Element items = j.next();
                    logger.debug(items.getName() + ":" + items.getText());
                    for (Iterator<Element> k = items.elementIterator("Item"); k
                            .hasNext();) {
                        Element item = k.next();
                        for (Iterator<Element> l = item
                                .elementIterator("Value"); l.hasNext();) {
                            Element Value = l.next();

                            int rowIndex = 0;// Row的索引
                            for (Iterator<Element> m = Value
                                    .elementIterator("Row"); m.hasNext();) {
                                Element row = m.next();
                                ...
                                if (0 == rowIndex) {
                                    ...
                                    for (Iterator<Element> n = row
                                            .elementIterator("Data"); n
                                            .hasNext();) {
                                        Element data = n.next();
                                ...
                                if (1 == rowIndex) {
                                    ...
                                    fieldStr = fieldStr.substring(0,
                                            fieldStr.length() - 1);
                                    resultVO.setTargetFields(fieldStr
                                            .split(","));
                                ...
                                for (Iterator<Element> n = row
                                        .elementIterator("Data"); n.hasNext();) {
                                    Element data = n.next();
                                    mapRow.put(
                                            resultVO.getTargetFields()[dataIndex],
                                            data.getStringValue());

                                    logger.debug("Row_Column[" + (rowIndex + 1)
                                            + "," + (dataIndex + 1) + "]:"
                                            + data.getStringValue());
                                    dataIndex++;
                                }
    ...
    public static List<Map<String, String>> parserResultVO(
            InfoResultVO infoResultVO) throws BaseAppException {
        if (!"000".equals(infoResultVO.getTargetCode())) {
            // 异常
            throw ExceptionHandler.publish("APP-01-0011",
                    "errorCode=" + infoResultVO.getTargetCode() + ",Msg="
                            + infoResultVO.getTargetMsg());
        }
