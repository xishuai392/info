...
Ext.onReady(function() {
    ...
    var thizAction = Ext.create("component..permission.action.AuditOrganizationAction");

    // 数据源
    thizStore = Ext.create('component..permission.store.AuditOrganizationStore', {
        // 定义分页大小
    ...
    var fbar = Ext.create("ZTEsoft.toolbar.ButtonsToolbar", {
        ...
        onClickAdd : function() {
            ...
            thizDetailForm.getForm().findField("orgId").selectText();
        },
        ...
        onClickDel : function() {
            ...
            var fieldPK = item.get('orgId');// TODO 获取记录的主键，后台根据主键删除该记录
        ...
        onClickSave : function() {
            ...
            if (actionType == WEBConstants.ACTIONTYPE.NEW) {
                thizAction.addRecord(params, function(result) {
                    ...
                    var thizModel = Ext.create("component..permission.model.AuditOrganizationModel");
             ...
             */
                ...
                var item = thizGrid.getSelectedItem();
                params.orgId = item.get("orgId");
                thizAction.modRecord(params, function(result) {
                    ExtUtils.info(StrConstants.HINT_MOD_SUCCESS);
                    // TODO  设置要更新的字段
                    var changedColumns = ['parentOrgId' ,'orgCode' ,'orgName' ,'orgLevel' ,'createdTime' ,'orgDesc' ,'state' ,'stateTime' ];
